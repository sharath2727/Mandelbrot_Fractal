...
main (int argc, char* argv[])
{
  ...
  //Logic is implemented here for sending blocks(rows) -start
  if(rank < remainder){
    starty+=(rank*(blocksize+1)*it);
  }else{
    starty = starty + (remainder*(blocksize+1)*it) + (rank-remainder)*blocksize*it;
  }
  ...
  if(rank == 0){
    ...
    gil::png_write_view("mandelbrot_mpi_block.png",const_view(img));
  }
